...
var app = new Vue({
	...
	methods : {
		...
		getTeamData : function() {
			...
			formData.append('dbType', 'MyBatis'); 
			const that = this
			axios.post(getTeamUrl, formData).then(function(res) {
				if (res.data.state == 0) {
...
$(function() {
	...
	$('#inputModal').on('show.bs.modal', function(event) {
		...
		if (type == 'edit') {
			var playerId = button.closest('tr').find('td').eq(1).text();
			var playerName = button.closest('tr').find('td').eq(2).text();
			var playerNum = button.closest('tr').find('td').eq(3).text();
			var teamId = button.closest('tr').find('td').eq(4).attr('value');
			var playerStuno = button.closest('tr').find('td').eq(5).text();
			var playerDepart = button.closest('tr').find('td').eq(6).text();
			var playerTel = button.closest('tr').find('td').eq(7).text();
			modal.find('.modal-body #playerId').val(playerId)
			modal.find('.modal-body #playerName').val(playerName)
			modal.find('.modal-body #playerNum').val(playerNum)
			modal.find('.modal-body #playerStuno').val(playerStuno)
			modal.find('.modal-body #playerDepart').val(playerDepart)
